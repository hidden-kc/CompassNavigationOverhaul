...
namespace hooks
{
	namespace
	{
		// The quest target the game's UpdateQuests loop is adding a marker for, matched back to
		// the owning quest and tracked objective by comparing it against every target of every
		// objective the player currently tracks. The resolution has to happen while the hook is
		// on the stack - aQuestTarget is only valid for the duration of this AddMarker call -
		// which is why the deferred path below resolves here and stores the result instead of
		// resolving again later.
		struct QuestTargetInfo
		{
			RE::TESQuest* quest = nullptr;
			RE::BGSInstancedQuestObjective* questObjective = nullptr;
			int questAgeIndex = -1;
		};

		QuestTargetInfo ResolveQuestTarget(const RE::TESQuestTarget* a_questTarget)
		{
			if (!a_questTarget)
			{
				return {};
			...
			RE::TESQuestTarget* questObjectiveTarget = reinterpret_cast<RE::TESQuestTarget*>(a_questTarget->unk00);
			...
			for (int ageIndex = playerObjectives.size() - 1; ageIndex >= 0; ageIndex--)
			{
				...
				for (int j = 0; j < questObjective->numTargets; j++)
				{
					...
					{
						return { questObjective->ownerQuest, playerObjective, ageIndex };
					}
				}
			}
					  ...
					  RE::TESQuestTarget* a_questTarget)
	{
		...
		// hook running (the reference unloading, a cell transition, etc). In the added case
		// AddMarker has already committed this marker to the scaleform data above, so the marker
		// still shows on the compass - we just cannot do the extra quest-objective bookkeeping
		// below without a live reference to read from. In the declined case there is nothing
		// to defer either, and warning would spam once per target per frame under a limit mod.
		if (!marker)
		{
			if (added)
			{
		...
		const QuestTargetInfo target = ResolveQuestTarget(a_questTarget);

		if (added)
		{
			if (target.quest)
			{
				CNO::HUDMarkerManager::GetSingleton()->ProcessQuestMarker(target.quest, target.questObjective,
																		  target.questAgeIndex, marker, a_markerGotoFrame,
																		  a_hudMarkerManager->currentMarkerIndex - 1);
			}

			return true;
		}

		// AddMarker declined this marker. A marker-limit mod (Quest Marker Limit Fix) makes
		// exactly this happen for every quest target: it swallows the calls, picks the closest
		// markers itself, then replays the winners straight into the original AddMarker - calls
		// this call-site hook never sees. Stash the target with the position the game handed us
		// (copied by value; the pointer is only valid for this call); if someone else really did
		// commit it this frame, ReconcilePendingQuestMarkers() finds that position in the marker
		// array at the end of the frame and runs the same bookkeeping against the slot it landed
		// in. Without such a mod the marker was genuinely not added, the position matches nothing,
		// and the pending entry is dropped.
		if (target.quest)
		{
			CNO::HUDMarkerManager::GetSingleton()->DeferQuestMarker(marker, *a_pos, target.quest, target.questObjective,
																	target.questAgeIndex, a_markerGotoFrame);
		}
